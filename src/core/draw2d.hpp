...
namespace vdj 
{
    ...
    /*
        ...
        Close = 255,
    */
    INLINE void strokePath(PixelView& pb, const PixelPath& path, const PixelRGBA &c, size_t width = 1)
    {
        ...
        Point fLastPosition;
        bool inFigure = false;
        GeoPolygon<ptrdiff_t> fig;

        for (size_t i = 0; i < path.fCommands.size(); i++)
        {
            switch (path.fCommands[i])
            {
            case PixelPath::ContourCommand::MoveTo:
            {
                // if already inFigure
                // then stroke that polygon
                if (inFigure) {
                    strokePolygon(pb, fig, c, width);
                }
                ...
                fLastPosition = path.getPoint(ptidx);
                ptidx += 2;
                fig.addPoint(fLastPosition);


            }
                break;

            case PixelPath::ContourCommand::Line:
            case PixelPath::ContourCommand::HLine:
            case PixelPath::ContourCommand::VLine:
            {
                // independent lines
            ...
            case PixelPath::ContourCommand::LineTo:
            {
                if (inFigure) {
                    fLastPosition = path.getPoint(ptidx);
                    fig.addPoint(fLastPosition);
            ...
            case PixelPath::ContourCommand::HLineTo:
            {
                // get x
            ...
            case PixelPath::ContourCommand::VLineTo:
            {
