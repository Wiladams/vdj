...
namespace glib
{
    ...
    struct multiplier_rgba
    {
        ...
        //--------------------------------------------------------------------
        static INLINE void premultiply(value_type* p)
        {
        ...
        //--------------------------------------------------------------------
        static INLINE void demultiply(value_type* p)
        {
            value_type a = p[Order::A];
    ...
    public:
        ...
        INLINE void operator () (value_type* p)
        {
    ...
    struct conv_rgba_pre
    {
        ...
        //--------------------------------------------------------------------
        static INLINE void set_plain_color(value_type* p, color_type c)
        {
            c.premultiply();
            p[Order::R] = c.r;
            p[Order::G] = c.g;
        ...
        //--------------------------------------------------------------------
        static INLINE color_type get_plain_color(const value_type* p)
        {
            return color_type(
                p[Order::R],
                p[Order::G],
    ...
    struct blender_rgba : conv_rgba_pre<ColorT, Order>
    {
        ...
        //--------------------------------------------------------------------
        static INLINE void blend_pix(value_type* p,
    ...
    struct comp_op_rgba_dst : blender_base<ColorT, Order>
    {
        ...
        // Da'  = Da.Sa + Da.(1 - Sa) = Da
        static INLINE void blend_pix(value_type*,
    ...
    struct comp_op_rgba_overlay : blender_base<ColorT, Order>
    {
        ...
        // 
        // Da' = Sa + Da - Sa.Da
        static INLINE double calc(double dca, double sca, double da, double sa, double sada, double d1a, double s1a)
        {
            return (2 * dca <= da) ?
    ...
    struct comp_adaptor_rgba
    {
        ...
        static INLINE void blend_pix(unsigned, value_type* p,
            value_type r, value_type g, value_type b, value_type a, cover_type cover)
        {
    ...
    private:
        //--------------------------------------------------------------------
        INLINE void blend_pix(pixel_type* p, const color_type& c, unsigned cover)
        {
            m_blender.blend_pix(p->c, c.r, c.g, c.b, c.a, cover);
        }

        //--------------------------------------------------------------------
        INLINE void blend_pix(pixel_type* p, const color_type& c)
        {
            m_blender.blend_pix(p->c, c.r, c.g, c.b, c.a);
        }

        //--------------------------------------------------------------------
        INLINE void copy_or_blend_pix(pixel_type* p, const color_type& c, unsigned cover)
        {
            if (!c.is_transparent())
            {
        ...
        //--------------------------------------------------------------------
        INLINE void copy_or_blend_pix(pixel_type* p, const color_type& c)
        {
            if (!c.is_transparent())
            {
                if (c.is_opaque())
    ...
    public:
        ...
        //--------------------------------------------------------------------
        INLINE unsigned width()  const { return m_rbuf->width(); }
        INLINE unsigned height() const { return m_rbuf->height(); }
        INLINE int      stride() const { return m_rbuf->stride(); }

        //--------------------------------------------------------------------
        INLINE       uint8_t* row_ptr(int y) { return m_rbuf->row_ptr(y); }
        INLINE const uint8_t* row_ptr(int y) const { return m_rbuf->row_ptr(y); }
        INLINE row_data     row(int y)     const { return m_rbuf->row(y); }

        //--------------------------------------------------------------------
        INLINE uint8_t* pix_ptr(int x, int y)
        {
            return m_rbuf->row_ptr(y) + sizeof(value_type) * (x * pix_step);
        }

        INLINE const uint8_t* pix_ptr(int x, int y) const
        {
            return m_rbuf->row_ptr(y) + sizeof(value_type) * (x * pix_step);
        }

        // Return pointer to pixel value, forcing row to be allocated.
        INLINE pixel_type* pix_value_ptr(int x, int y, unsigned len)
        {
        ...
        INLINE const pixel_type* pix_value_ptr(int x, int y) const
        {
            uint8_t* p = m_rbuf->row_ptr(y);
            return p ? (pixel_type*)(p + sizeof(value_type) * (x * pix_step)) : 0;
        }

        // Get pixel pointer from raw buffer pointer.
        INLINE static pixel_type* pix_value_ptr(void* p)
        {
            return (pixel_type*)p;
        }

        // Get pixel pointer from raw buffer pointer.
        INLINE static const pixel_type* pix_value_ptr(const void* p)
        {
            return (const pixel_type*)p;
        }

        //--------------------------------------------------------------------
        INLINE static void write_plain_color(void* p, color_type c)
        {
            blender_type::set_plain_color(pix_value_ptr(p)->c, c);
        }

        //--------------------------------------------------------------------
        INLINE static color_type read_plain_color(const void* p)
        {
            return blender_type::get_plain_color(pix_value_ptr(p)->c);
        }

        //--------------------------------------------------------------------
        INLINE static void make_pix(uint8_t* p, const color_type& c)
        {
            ((pixel_type*)p)->set(c);
        }
        ...
        //--------------------------------------------------------------------
        INLINE void copy_pixel(int x, int y, const color_type& c)
        {
            pix_value_ptr(x, y, 1)->set(c);
        }

        //--------------------------------------------------------------------
        INLINE void blend_pixel(int x, int y, const color_type& c, uint8_t cover)
        {
            copy_or_blend_pix(pix_value_ptr(x, y, 1), c, cover);
        }

        //--------------------------------------------------------------------
        INLINE void copy_hline(int x, int y,
            unsigned len,
            const color_type& c)
        {
            pixel_type v;
        ...
        //--------------------------------------------------------------------
        INLINE void copy_vline(int x, int y,
            unsigned len,
            const color_type& c)
        {
            pixel_type v;
        ...
        template<class RenBuf2> void copy_from(const RenBuf2& from,
            ...
            unsigned len)
        {
            if (const uint8_t* p = from.row_ptr(ysrc))
            {
    ...
    private:
        //--------------------------------------------------------------------
        INLINE void blend_pix(pixel_type* p, const color_type& c, unsigned cover = cover_full)
        {
            m_blender.blend_pix(m_comp_op, p->c, c.r, c.g, c.b, c.a, cover);
        }

        //--------------------------------------------------------------------
        INLINE void copy_or_blend_pix(pixel_type* p, const color_type& c, unsigned cover = cover_full)
        {
            if (!c.is_transparent())
            {
                if (c.is_opaque() && cover == cover_mask)
    ...
    public:
        ...
        void blend_hline(int x, int y, unsigned len,
            const color_type& c, uint8_t cover)
        {
        ...
        void blend_solid_hspan(int x, int y, unsigned len,
            const color_type& c, const uint8_t* covers)
        {
            pixel_type* p = pix_value_ptr(x, y, len);
