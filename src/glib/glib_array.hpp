...
namespace glib
{
    ...
    public:
        ...
        void deserialize(const uint8_t* data, unsigned byte_size);
        const T& operator [] (unsigned i) const { return m_array[i]; }
        T& operator [] (unsigned i) { return m_array[i]; }
        const T& at(unsigned i) const { return m_array[i]; }
        T& at(unsigned i) { return m_array[i]; }
    ...
    //------------------------------------------------------------------------
    template<class T> void pod_vector<T>::serialize(uint8_t* ptr) const
    {
    ...
    //------------------------------------------------------------------------
    template<class T>
    void pod_vector<T>::deserialize(const uint8_t* data, unsigned byte_size)
    {
        byte_size /= sizeof(T);
        allocate(byte_size);
    ...
    public:
        ...
        void deserialize(unsigned start, const T& empty_val,
            const uint8_t* data, unsigned byte_size);
    ...
    //------------------------------------------------------------------------
    template<class T, unsigned S>
    void pod_bvector<T, S>::serialize(uint8_t* ptr) const
    {
        unsigned i;
        for (i = 0; i < m_size; i++)
        {
    ...
    //------------------------------------------------------------------------
    template<class T, unsigned S>
    void pod_bvector<T, S>::deserialize(const uint8_t* data, unsigned byte_size)
    {
        remove_all();
        byte_size /= sizeof(T);
    ...
    public:
        void remove_all()
        {
            if (m_num_blocks)
            {
                ...
                while (m_num_blocks--)
                {
                    pod_allocator<uint8_t>::deallocate(blk->data, blk->size);
                    --blk;
                }
        ...
        uint8_t* allocate(unsigned size, unsigned alignment = 1)
        {
            if (size == 0) return 0;
            if (size <= m_rest)
            {
                uint8_t* ptr = m_buf_ptr;
                if (alignment > 1)
                {
                    unsigned align =
    ...
    private:
        void allocate_block(unsigned size)
        {
            ...
            m_blocks[m_num_blocks].data =
                m_buf_ptr =
                pod_allocator<uint8_t>::allocate(size);
