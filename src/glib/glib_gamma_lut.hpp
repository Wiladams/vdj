...
#include "glib_gamma_functions.hpp"

namespace glib
{
    template<class LoResT = uint8_t,
        class HiResT = uint8_t,
    ...
    template<>
    class sRGB_lut<uint16_t> : public sRGB_lut_base<uint16_t>
    {
    public:
        sRGB_lut()
        {
    ...
    template<>
    class sRGB_lut<uint8_t> : public sRGB_lut_base<uint8_t>
    {
    public:
        sRGB_lut()
        {
    ...
    {
    public:
        static T rgb_from_sRGB(uint8_t x)
        {
            return lut.dir(x);
        }

        static uint8_t rgb_to_sRGB(T x)
        {
            return lut.inv(x);
        }
    ...
    {
    public:
        static float alpha_from_sRGB(uint8_t x)
        {
            static const double y = 1 / 255.0;
            return float(x * y);
        }

        static uint8_t alpha_to_sRGB(float x)
        {
            if (x < 0) return 0;
            if (x > 1) return 255;
            return uint8_t(0.5 + x * 255);
        }
    };

    template<>
    class sRGB_conv<uint16_t> : public sRGB_conv_base<uint16_t>
    {
    public:
        static uint16_t alpha_from_sRGB(uint8_t x)
        {
            return (x << 8) | x;
        }

        static uint8_t alpha_to_sRGB(uint16_t x)
        {
            return x >> 8;
        }
    };
    ...
    template<>
    class sRGB_conv<uint8_t> : public sRGB_conv_base<uint8_t>
    {
    public:
        static uint8_t alpha_from_sRGB(uint8_t x)
        {
            return x;
        }

        static uint8_t alpha_to_sRGB(uint8_t x)
        {
            return x;
        }
    };
